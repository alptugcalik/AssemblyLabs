...
MSG					DCB	"Give the duty cycle (0-9):",0x04
...
__main proc
	...
	; PB6 is used for pwm 
			; PB4 is used for ECHO
			; PB5 is connected to TRIG
    bl pwm_init
	BL DELAY1
	BL GPIOINIT_B
	BL DELAY1
	BL INIT_TIMER1 ; EDGE TIME 
...
input_loop
	...
	; TRIGGER THE SOUND MODULE
			LDR R1, =GPIO_PORTB_DATA
			LDR R0, [R1] 
			ORR R0, R0, #0x20 				; SET PB5
			STR R0, [R1] 
			BL DELAY1
			LDR R1, =GPIO_PORTB_DATA
			LDR R0, [R1] 
			BIC R0, R0, #0x20 			; CLEAR PB5 	
			STR R0, [R1] 
			...
			;BHI  input_loop
			;MOV R1,#4000 
			;MUL R0,R1 ; FIND THE MATCH VALUE 
			;MOV R1,#10
			;UDIV R0,R1
			;LDR R1, =TIMER0_MATCH ; match 
			;MOV R2,R0
			;STR R2, [R1]
	b input_loop
